...
class EventsViewModel: ObservableObject {
    ...
    func addEvent(event: UserEvent, viewController: UIViewController) {
        ...
        // Create Apple Calendar event and present the event editor
        ...
        eventStore.requestAccess(to: .event) { granted, error in
            ...
            if granted {
                DispatchQueue.main.async {
                    ...
                    // Pre-fill the event and allow the user to add invitees
                    let eventEditViewController = EKEventEditViewController()
                    eventEditViewController.event = calendarEvent
                    eventEditViewController.eventStore = eventStore
                    eventEditViewController.editViewDelegate = viewController as? EKEventEditViewDelegate
                    viewController.present(eventEditViewController, animated: true) {
                        print("Event editor presented for adding invitees.")
                    }
                }
            } else {
